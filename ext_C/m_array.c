...
VALUE bigFib(VALUE obj) {
  unsigned long long i=0, range=NUM2ULL(obj);
...
void Init_m_array() {
  VALUE cArr = rb_ary_new();
  rb_funcall(cArr, rb_intern("<<"), 1, INT2FIX(1));
  rb_ary_push(cArr, INT2FIX(2));
  rb_ary_push(cArr, Qfalse);

  rb_global_variable(&cArr);
  rb_gv_set("$a", cArr);

  // Add a new method into class Integer:
  ...
  rb_define_method(_integer, "bigFib", bigFib, 0);
